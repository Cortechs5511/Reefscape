...
public class SwerveModule {
    ...
    private SimpleMotorFeedforward driveFeedforward = new SimpleMotorFeedforward(SwerveConstants.kSDrive, SwerveConstants.kVDrive, SwerveConstants.kADrive);
    ...
    public void setTargetState(SwerveModuleState targetState, PIDController drivePID, ProfiledPIDController turnPID) {
        ...
        double driveOutput = setSpeed(targetState, drivePID);

        // multiply output by 12 because we are using setVoltage instead of set
        // this will ensure a consistent voltage will always be sent to the motor, regardless of current battery voltage
        turnMotor.setVoltage(turnOutput * 12);
        driveMotor.setVoltage(driveOutput * 12);
    }

    public double setSpeed(SwerveModuleState targetState, PIDController drivePID) {
        double driveOutput = driveFeedforward.calculate(targetState.speedMetersPerSecond);
        driveOutput += drivePID.calculate(getVelocity(), targetState.speedMetersPerSecond);
        return driveOutput;
    }
    ...
    public double getVelocityMetersPerSecond() {
        return driveEncoder.getVelocity() * SwerveConstants.VELOCITY_CONVERSION_FACTOR;
    }

    // configuring class types
    ...
    private RelativeEncoder createEncoder(SparkMax controller) {
        ...
        // encoder.velocityConversionFactor(SwerveConstants.VELOCITY_CONVERSION_FACTOR);
